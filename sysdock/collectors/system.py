...
def _sysctl(key):
    ...
    try:
        out = subprocess.check_output(["sysctl", "-n", key], timeout=2, stderr=subprocess.DEVNULL)
...
def get_hostname():
    ...
    if _IS_WINDOWS:
        ...
        os_name = f"macOS {ver[0]}" if ver[0] else "macOS"
        ...
        return [
            {
                ...
                "mem_pct": 0.0,
                "temp_c": 0.0,
                "driver_ver": platform.mac_ver()[0]
                if getattr(platform, "mac_ver", None)
                else "Metal",
            }
        ]
    except Exception:
