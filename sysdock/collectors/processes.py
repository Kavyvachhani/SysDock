        ...
        try:
            for proc in _psutil.process_iter(["status"]):
                try:
                    counts["total"] += 1
                    s = proc.info["status"]
                    if s == _psutil.STATUS_RUNNING:
                        counts["running"] += 1
                    elif s == _psutil.STATUS_ZOMBIE:
                        counts["zombie"] += 1
                    elif s == _psutil.STATUS_STOPPED:
                        counts["stopped"] += 1
                    else:
                        counts["sleeping"] += 1
                except Exception:
            ...
            with open(path) as f:
                for line in f.readlines()[1:]:
                    ...
                    try:
                        ...
                        ip = ip_hex
                    ports.append({"local_address": f"{ip}:{port}", "process": ""})
        except Exception:
            pass
    return ports
