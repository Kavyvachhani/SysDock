...
def _header(sys_d):
    ...
    return Panel(
        g,
        title=f"[bold cyan]{TOOL_NAME} v{VERSION}[/bold cyan]",
        border_style="cyan",
        padding=(0, 1),
    )
...
def _cpu_panel(sys_d, dkr_d, show_cores=True):
    ...
    global_t.add_row(
        Text("ALL", style="bold"), _bar(total, 26), Text(f"{total:5.1f}%", style=_pct_style(total))
    )
    ...
    if show_cores:
        ...
        for r in range(rows_needed):
            ...
            if c1_idx < num_cores:
                ...
                row_items.extend(
                    [
                        Text(f"{c1_idx:2d} ", style="dim"),
                        _bar(p1, 10),
                        Text(f" {p1:>5.1f}%", style=_pct_style(p1)),
                    ]
                )
            if c2_idx < num_cores:
                ...
                row_items.extend(
                    [
                        Text(f"  {c2_idx:2d} ", style="dim"),
                        _bar(p2, 10),
                        Text(f" {p2:>5.1f}%", style=_pct_style(p2)),
                    ]
                )
    ...
    if dkr_cpu > 0:
        ...
        attr_t.add_column(width=6)
        attr_t.add_column(ratio=1)
        attr_t.add_column(width=8, justify="right")
        attr_t.add_row(
            Text("🐳", style="cyan"),
            _bar(min(100.0, dkr_cpu), 26),
            Text(f"~{dkr_cpu:.1f}%", style="cyan"),
        )
        attr_t.add_row(
            Text("OS", style="dim"),
            _bar(min(100.0, sys_cpu), 26),
            Text(f"~{sys_cpu:.1f}%", style="dim"),
        )
        main_grid.add_row(Text(""))
...
def _gpu_panel(sys_d):
    ...
    for idx, g in enumerate(gpus[:2]):  # show up to 2 GPUs
        ...
        if vendor == "Apple":
            ...
            t.add_row(
                Text("VRAM", style="bold"),
                _bar(m_pct if m_tot else 0, 18, 80, 90),
                Text(f"{m_used:.0f}/{m_tot:.0f}M" if m_tot else "Shared", style="dim"),
            )
            t.add_row(Text("Util", style="bold"), Text("(Metal API — not exposed)", style="dim"))
        else:
            t.add_row(
                Text("Load", style="bold"),
                _bar(u, 18, 70, 90),
                Text(f"{u:5.1f}%", style=_pct_style(u)),
            )
            t.add_row(
                Text("Mem", style="bold"),
                _bar(m_pct, 18, 80, 90),
                Text(f"{m_pct:5.1f}%", style=_pct_style(m_pct)),
            )
            if temp:
                t.add_row(
                    Text("Temp", style="bold"),
                    _bar(min(100, temp), 18, 75, 85),
                    Text(f"{temp:.0f}°C", style="yellow"),
                )

        if m_tot or driver_ver != "Unknown":
            sub_text = []
            if m_tot:
                sub_text.append(f"{m_used:.0f}/{m_tot:.0f} MB")
            if driver_ver and driver_ver != "Unknown":
                sub_text.append(f"Driver: {driver_ver}")
...
def _mem_panel(sys_d, dkr_d):
    ...
    t.add_row(
        ...
        _bar(rp, 20),
        Text(
            "{} / {} {:5.1f}%".format(_mb(m.get("used_mb")), _mb(m.get("total_mb")), rp),
            style=_pct_style(rp),
        ),
    )
    ...
    t.add_row(
        ...
        _bar(sp, 20, 50, 80),
        Text(
            "{} / {} {:5.1f}%".format(_mb(m.get("swap_used_mb")), _mb(m.get("swap_total_mb")), sp),
    ...
    if dkr_mem_mb > 0:
        ...
        t.add_row(
            Text("🐳", style="cyan"),
            _bar(dkr_pct, 20),
            Text(f"{_mb(dkr_mem_mb)} ({dkr_pct:.1f}%)", style="cyan"),
        )
        ...
        t.add_row(
            Text("OS", style="dim"),
            _bar(sys_pct, 20),
            Text(f"{_mb(sys_mem_mb)} ({sys_pct:.1f}%)", style="dim"),
        )
...
def _disk_panel(disk_d, dkr_d):
    t = Table(
        box=box.SIMPLE, show_header=True, header_style="bold dim", show_edge=False, padding=(0, 1)
    )
    ...
    for p in disk_d.get("partitions", []):
        ...
        t.add_row(
            ...
            Text(f"{pct:>4.0f}%", style=_pct_style(pct, 75, 90)),
        )
    ...
    if dkr_d.get("available"):
        ...
        if dkr_disk:
            ...
            dkr_sub = f"  [cyan]🐳 docker — imgs:{imgs_sz} cntrs:{cntr_sz} vols:{vol_sz}[/cyan]"
        else:
            n_imgs = len(dkr_d.get("images", []))
            if n_imgs:
                total_img_mb = sum(
                    float(img.get("size_mb") or 0) for img in dkr_d.get("images", [])
                )
                dkr_sub = f"  [cyan]🐳 {n_imgs} images  ~{_mb(total_img_mb)} total[/cyan]"
...
def _net_panel(net_d):
    ...
    for iface in net_d.get("interfaces", []):
        ...
        t.add_row(
            ...
            ips or "—",
            Text(f"{rx:.3f}M", style="cyan" if rx > 0 else "dim"),
            Text(f"{tx:.3f}M", style="green" if tx > 0 else "dim"),
...
def _proc_panel(proc_d, limit=14):
    ...
    for p in procs[:limit]:
        ...
        t.add_row(
            ...
            Text(f"{cpu:5.1f}", style=_pct_style(cpu)),
            Text(f"{mem:5.1f}", style=_pct_style(mem)),
            _mb(p.get("rss_mb")),
            cmd,
        )
    sub = "[dim]tasks {}  run {}  slp {}  zmb {}[/dim]".format(
        ...
        summary.get("sleeping", 0),
        summary.get("zombie", 0),
    )
    return Panel(
        t, title="[bold]Processes[/bold] [dim](by CPU)[/dim]", subtitle=sub, border_style="blue"
    )
...
def _docker_panel(dkr_d, limit=14):
    ...
    for c in containers[:limit]:
        ...
        t.add_row(
            ...
            st_text,
            Text(f"{cpu:5.1f}", style=_pct_style(cpu)) if is_run else Text("—", "dim"),
            Text(mem_u) if is_run else Text("—", "dim"),
            Text(f"{mem:5.1f}", style=_pct_style(mem)) if is_run else Text("—", "dim"),
            Text(f"{float(rx or 0):.2f}M", style="cyan") if rx is not None else Text("—", "dim"),
            Text(f"{float(tx or 0):.2f}M", style="green") if tx is not None else Text("—", "dim"),
        )
...
def _sec_panel(sec_d):
    ...
    t.add_row(
        Text("SSH fail:", style="bold"), Text(str(fails), style="bold red" if fails else "green")
    )
    t.add_row(Text("SSH ok:", style="bold"), Text(str(ok), style="green"))

    f2b = sec_d.get("fail2ban", {})
    t.add_row(
        Text("fail2ban:", style="bold"),
...
def _ai_panel(proc_d):
    ...
    if not ai_procs:
        return Panel(
            Align.center(Text("No AI services detected", style="dim")),
            title="[bold]AI / Ollama[/bold]",
            border_style="dim",
        )
    ...
    for p in ai_procs[:6]:
        ...
        t.add_row(
            ...
            if sc == "R"
            else Text("○ " + str(p.get("status", "S"))[:4], style="dim"),
            Text(f"{cpu:5.1f}", style=_pct_style(cpu)),
            _mb(p.get("rss_mb")),
        )
...
def _render(state):
    ...
    for l in layout["middle"].children:
        l.debug = False
...
def run_dashboard(refresh=3.0):
    ...
    console.print(f"[bold cyan]{TOOL_NAME} v{VERSION} — initialising...[/bold cyan]")
